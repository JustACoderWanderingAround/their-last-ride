...
{
	std::vector<std::string> names = { "George", "Sasha", "Emily", "Montag", "Bill"}; // to be changed
	...
	{
		_seats[positions[rand() % no_of_positions]] = new InteractablePerson(names[rand() % (names.size() - 1)]);
	}
